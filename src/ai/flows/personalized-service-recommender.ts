...
'use server';
/**
 * @fileOverview This file implements a Genkit flow to recommend INFISAHAI services
...
const PersonalizedServiceRecommenderOutputSchema = z.object({
  recommendationSummary: z.string().describe('A personalized summary of how INFISAHAI services can address the business\'s needs.'),
  relevantServices: z.array(z.string()).describe('A list of specific INFISAHAI services most relevant to the business.'),
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert consultant for INFISAHAI, a company specializing in AI implementation, automation, and modern web development services.
Your goal is to analyze a business's details and challenges, and then provide a personalized summary of how INFISAHAI's services can specifically address their needs and provide significant value.

INFISAHAI offers the following core services:
...
Based on this, generate a comprehensive recommendation summary and list the most relevant INFISAHAI services that would benefit this business. Also, if possible, describe the estimated ROI and benefits they could expect.`,
});
